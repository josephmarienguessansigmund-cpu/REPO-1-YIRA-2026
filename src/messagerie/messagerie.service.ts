...
export class MessagerieService {
  ...
  async creerConversationMatching(params: {
    ...
    const { data, error } = await this.supabase
      ...
      .select()
      .single();
    if (error) throw new Error(`Erreur cr�ation conversation: ${error.message}`);
  ...
  async manifesterInteret(params: {
    ...
    const conv = await this.getConversation(params.conversation_id);
    if (!conv) throw new NotFoundException('Conversation introuvable');
    if (conv.drh_id !== params.drh_id) throw new ForbiddenException('Acc�s refus�');
    await this.supabase
      .from('YiraConversation')
    ...
    await this.supabase.from('YiraSmsLog').insert({
      ...
      contenu: 'YIRA: Un employeur est int�ress� par votre profil ! Votre conseiller vous contacte sous 24h.',
      statut: 'EN_ATTENTE',
      country_code: params.country_code,
    });
    return { candidat_revele: true, message: 'Profil complet d�bloqu�. Le conseiller YIRA coordonne la mise en relation.' };
  }
  ...
  async enregistrerDecision(params: {
    ...
    await this.envoyerMessage({
      ...
      contenu: params.decision === 'acceptee'
        ? `Candidature accept�e${params.motif ? ` � ${params.motif}` : ''}. Le contrat sera pr�par� sous 48h.`
        : `Candidature non retenue${params.motif ? ` � ${params.motif}` : ''}. Le candidat sera r�orient�.`,
      type_message: 'decision',
      country_code: params.country_code,
    });
    this.logger.log(`D�cision ? ${params.decision} � conversation ${params.conversation_id}`);
  }
