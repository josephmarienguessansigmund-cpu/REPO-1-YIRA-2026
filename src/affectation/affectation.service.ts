...
export class AffectationService {
  ...
  private readonly RIASEC_FAMILLES: Record<string, string[]> = {
    R: ['B�timent & Construction', 'Technique & M�canique'],
    I: ['Sant� & Social', 'Technologie & Num�rique'],
    A: ['Art & Cr�ativit�', 'Communication & M�dias'],
    S: ['�ducation & Formation', 'Sant� & Social'],
    E: ['Commerce & Gestion', 'Management & Leadership'],
    C: ['Comptabilit� & Finance', 'Administration & Secr�tariat'],
  };
  ...
  private async notifierEtablissement(etab: any, profil: ProfilOrientation) {
    await this.supabase.from('YiraSmsLog').insert({
      ...
      contenu: `YIRA: Candidat ${profil.code_yira} recommand� pour votre �tablissement. Contact: nohama@yira.ci`,
      statut: 'EN_ATTENTE',
      country_code: profil.country_code,
    });
  }
